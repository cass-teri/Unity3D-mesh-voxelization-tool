...
namespace Voxelization.Tools
{
    ...
    {
        private static readonly string appName = "Mesh Voxelization Tool";
...
/* Unmerged change from project 'MeshVoxelizerTool.Player'
Before:
                maxSide = FastMathUtils.Max(len.y, maxSide);
                maxSide = FastMathUtils.Max(len.z, maxSide);
After:
                maxSide = MathUtils.Max(len.y, maxSide);
                maxSide = MathUtils.Max(len.z, maxSide);
*/
                maxSide = Utils.MathUtils.Max(len.y, maxSide);
                maxSide = Utils.MathUtils.Max(len.z, maxSide);
...
After:
                        min = MathUtils.Min(filter.sharedMesh.bounds.min, min);
                        max = MathUtils.Max(filter.sharedMesh.bounds.max, max);
*/
                        min = Utils.MathUtils.Min(filter.sharedMesh.bounds.min, min);
                        max = Utils.MathUtils.Max(filter.sharedMesh.bounds.max, max);
                    }
                }
            }
